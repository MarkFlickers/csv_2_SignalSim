...
Единицы:
- lat/lon — **в градусах**
- alt — **в метрах** (если нет столбца высоты, берётся 0, а сегменты с VerticalAcc не добавляются)

**Внимание**: крайне нежелательно передавать траекторию, содержащую участки с нулевой горизонтальной скоростью (будь то вертикальный взлёт или стояние на месте). Из-за этого снижается точность воспроизведения и может сбиться курс (особенность SignalSim и перевода ENU -> ECEF в частности). 
...
2. очень короткое изменение курсовой скорости `ConstAcc` длительностью `eps_acc`
3. очень короткое изменение вертикальной скорости `VerticalAcc` длительностью `eps_acc` (Если в эпохах есть колонка `alt`)
4. движение по прямой `Const` на оставшееся время
...
Важно: должно выполняться `eps_turn + 2*eps_acc < dt`.
